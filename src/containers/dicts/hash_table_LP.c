...
//Can be compared using SIMD and the final result can be reduced to return the result
#define BYTEWISE_EQUALS(size, lhs, rhs)                                                                                \
    ({
        //128 bits are compared in an instant
        bool equals = true;                                                                                             \
                          ...
                          void (*cleanup)(void *key, void *value), bool key_is_str)
{
    ...
    *extra = (hash_table_extra_t) {
        ...
        .slots = GS_REQUIRE_MALLOC(num_slots * (key_size + elem_size)), //This has to be initilized as SIMD accessible array
